    ...
    try {
      ...
      if (fileList.length) {
        for (const f of fileList) {
          try {
            setMsg({ tone: "neutral", text: `⏳ Procesando foto: ${f.name} (${f.size} bytes, ${f.type})…` });
            let prepared;
            try {
              prepared = await prepareReceiptImage(f);
              setMsg({ tone: "neutral", text: `⏳ Foto comprimida OK: ${prepared.blob?.size} bytes. Guardando…` });
            } catch (compressErr) {
              setMsg({ tone: "warning", text: `⚠️ Compresión fallida: ${compressErr?.message}. Usando original…` });
              const contentHash = await import("../services/crypto.js").then(m => m.sha256Hex(f)).catch(() => "");
              prepared = {
                blob: f,
                mimeType: f.type || "image/jpeg",
                filename: f.name || "boleta.jpg",
                width: null,
                height: null,
                contentHash,
              };
    ...
    <div className="card">
      ...
      <div className="row row-form" style={{ marginTop: 12 }}>
        <SelectField
          ...
          options={[{ value: "", label: "—" }, ...parts.map((x) => ({ value: x.partidaCodigo, label: `${x.partidaCodigo} - ${x.partidaNombre}` }))]}
        ...
        <SelectField
          ...
          options={[{ value: "", label: "—" }, ...clasifs.map((x) => ({ value: x.clasificacionCodigo, label: `${x.clasificacionCodigo} - ${x.clasificacionNombre}` }))]}
