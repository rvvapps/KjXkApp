...
export default function Settings() {
  ...
  const [restoreMsg, setRestoreMsg] = useState("");  // FIX: era setRestoreStatus (no definida)
  ...
  async function doRestoreBackup() {
    ...
    try {
      ...
      // FIX: onProgress ahora llama a setRestoreMsg (antes llamaba a setRestoreStatus, que no existía)
      const r = await restoreFromEncryptedBackupFile(restoreFile, restorePass, {
        timeoutMs: 60000,
        onProgress: (p) => {
          setRestoreMsg(formatProgress(p));
        },
      });
      ...
      } else if (String(code).includes("timeout")) {
        setRestoreMsg("❌ Tiempo de espera agotado. Intenta de nuevo.");
      } else {
  ...
  return (
    <div className="card">
      ...
      {restoreMsg && (
        <div className="small" style={{ marginTop: 10, padding: 10, border: "1px solid rgba(255,255,255,.12)", borderRadius: 12 }}>
          {typeof restoreMsg === "string" ? restoreMsg : formatProgress(restoreMsg)}
        </div>
      )}
