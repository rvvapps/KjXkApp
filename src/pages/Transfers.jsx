...
export default function Transfers() {
  ...
  const [editModal, setEditModal] = useState(null); // traslado que se está editando
  const [editForm, setEditForm] = useState({});
  const [editMsg, setEditMsg] = useState("");

  async function handleDeleteTransfer(transferId) {
    if (!window.confirm("¿Eliminar este traslado?")) return;
    try {
      setBusy(true);
      await deleteTransfer(transferId);
      setItems((prev) => prev.filter((t) => t.transferId !== transferId));
      setSelected((prev) => { const s = new Set(prev); s.delete(transferId); return s; });
    } catch (e) {
      setMsg("Error al eliminar: " + String(e));
    } finally {
      setBusy(false);
    }
  }
  ...
  function openEdit(t) {
    setEditForm({
      fecha: t.fecha ? t.fecha.slice(0, 10) : new Date().toISOString().slice(0, 10),
      destino: t.destino || "",
      destinationId: t.destinationId || "",
      tipo: t.tipo || "Vehículo propio",
      crCodigo: t.crCodigo || "",
      visita: t.visita || "",
      monto: t.monto ? String(t.monto) : "",
      notas: t.notas || "",
    });
    setEditMsg("");
    setEditModal(t);
  ...
  async function saveEdit() {
    if (!editForm.destino.trim()) return setEditMsg("Ingresa el destino.");
    if (!editForm.crCodigo) return setEditMsg("Selecciona CR.");
    if (!editForm.visita.trim()) return setEditMsg("Ingresa la visita/viaje.");
    const payload = {
      ...editModal,
      fecha: new Date(editForm.fecha + "T12:00:00").toISOString(),
      destino: editForm.destino.trim(),
      destinationId: editForm.destinationId || null,
      tipo: editForm.tipo,
      crCodigo: editForm.crCodigo,
      visita: editForm.visita.trim(),
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        ...
                        <button
                          ...
                          style={{ padding: "4px 10px", fontSize: 13 }}
                          disabled={busy}
                          onClick={() => handleDeleteTransfer(t.transferId)}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ margin: 0 }}>✏️ Editar trayecto</h3>
              <button onClick={() => setEditModal(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--text3)" }}>✕</button>
            </div>

            <TextField label="Fecha" type="date" value={editForm.fecha}
              onChange={v => setEditForm(f => ({ ...f, fecha: v }))} />
            <TextField label="Destino" value={editForm.destino}
              onChange={v => setEditForm(f => ({ ...f, destino: v }))} />
            <SelectField label="Tipo" value={editForm.tipo}
              onChange={v => setEditForm(f => ({ ...f, tipo: v }))}
              options={TRANSFER_TYPES.map(t => ({ value: t, label: t }))} />
            <SelectField label="CR" value={editForm.crCodigo}
              onChange={v => setEditForm(f => ({ ...f, crCodigo: v }))}
              options={crs.map(c => ({ value: c.codigo, label: `${c.codigo} — ${c.nombre}` }))} />
            <TextField label="Visita / Viaje" value={editForm.visita}
              onChange={v => setEditForm(f => ({ ...f, visita: v }))} />
            <TextField label="Monto ($)" type="number" value={editForm.monto}
              onChange={v => setEditForm(f => ({ ...f, monto: v }))} />
            <TextField label="Notas" value={editForm.notas}
              onChange={v => setEditForm(f => ({ ...f, notas: v }))} />

            {editMsg && <div style={{ color: "var(--danger)", fontSize: 13 }}>{editMsg}</div>}
