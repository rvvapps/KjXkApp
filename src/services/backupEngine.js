...
export async function restoreFromEncryptedBackupFile(fileBlob, passphrase, opts = {}) {
  ...
  // no necesitan propagarse por sync. Evita que eventos viejos generen fantasmas.
  try {
    const now = new Date().toISOString();
    // Marcar todos los eventos del outbox como ya subidos — status:"uploaded" es lo que
    // listPendingOutbox usa para filtrar. uploadedAt solo no era suficiente.
    const allOutbox = await db.getAll("sync_outbox").catch(() => []);
    for (const ev of allOutbox) {
      await db.put("sync_outbox", { ...ev, status: "uploaded", uploadedAt: now }).catch(() => {});
    }
    await db.clear("sync_inbox").catch(() => {});
  } catch (e) {
    console.warn("restore: no se pudo limpiar outbox/inbox", e);
  }
