...
// Mantener compatibilidad con el slicing histórico en Reimbursements.jsx
const CAPACITY = 42;
...
/**
 * Devuelve un Blob del Excel (para snapshot al aprobar).
 * items: array enriquecido:
 * { docTipo, fechaISO, docNumero, detalle, crCodigo/crNombre, ctaCodigo/ctaNombre, partidaCodigo/partidaNombre, clasificacionCodigo/clasificacionNombre, monto }
 */
export async function generateBatchXlsxBlob({ correlativo, headerOverrides = {}, items }) {
  ...
  ["A3", "A4", "A5", "A6"].forEach((addr) => (ws.getCell(addr).font = { bold: true }));
 ...
 * Devuelve items enriquecidos con nombres desde catálogos para Excel:
