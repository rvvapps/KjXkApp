...
export async function generateBatchXlsxBlob({ correlativo, headerOverrides = {}, items, tipoRendicion }) {
  ...
  // ── Configuración de página: Carta horizontal, 2 páginas exactas ───────────
  // fitToHeight:2 + fitToWidth:1 hace que Excel escale para caber en 2 páginas
...
export async function exportBatchXlsx({ correlativo, headerOverrides = {}, items, tipoRendicion }) {
  try {
    ...
    downloadBlob(blob, `${yy}_${mm}_${dd} Rendicion ${correlativo || "SinNumero"}.xlsx`);
    return blob;
  } catch (err) {
