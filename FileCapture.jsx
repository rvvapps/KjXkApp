    ...
    try {
      for (const f of files) {
        ...
        try {
          ...
          console.warn("Compresión fallida, usando original:", compressErr);
          const { sha256Hex } = await import("../services/crypto.js");
          const contentHash = await sha256Hex(f).catch(() => "");
          prepared = { blob: f, mimeType: f.type || "image/jpeg", filename: f.name || "boleta.jpg", width: null, height: null, contentHash };
        }
      ...
      console.error(e);
      setMsg(`Error agregando respaldo: ${e?.message || String(e)}`);
    } finally {
      setBusy(false);
    }
  }
